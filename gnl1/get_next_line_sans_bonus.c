...
/*   Updated: 2018/03/01 14:48:36 by lballiot         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	ft_read(t_struct *list, char *tmp, char **line, const int fd)
{
	...
	while ((list->i = read(fd, buf, BUFF_SIZE)) != EOF && list->i > 0)
	{
		...
		{
			if (ft_find_first_char(buf, CHAR) > 0)
			{
				cpy = ft_strsub(buf, 0, ft_find_first_char(buf, CHAR));
				tmp = ft_strjoin_and_free(tmp, cpy);
				ft_strdel(&cpy);
			}
			*line = ft_strdup(tmp);
			ft_strdel(&tmp);
			if (ft_strstr(buf, TO_FIND))
				list->str = ft_strdup(ft_strstr(buf, TO_FIND));
			ft_strdel(&buf);
			return (1);
		}
...
{
	if (ft_find_first_char(l->str, CHAR) > 0)
		tmp = ft_strsub(l->str, 0, ft_find_first_char(l->str, CHAR));
	*line = ft_strdup(tmp);
	if (ft_strstr(l->str, TO_FIND))
		l->str = ft_strdup(ft_strstr(l->str, TO_FIND));
	return (1);
}
...
int			get_next_line(const int fd, char **line)
{
	...
	if (l.str)
	{
		if ((l.str[0] == CHAR && l.str[1] == CHAR && l.str[2] == '\0'))
		{
			*line = "";
			l.str = ft_strsub(l.str, 1, (ft_strlen(l.str) - 2));
			return (1);
		}
		if ((j = ft_find_last_char(l.str, CHAR)) > 0)
			l.str = ft_strsub(l.str, j, (ft_strlen(l.str) - j));
		if (ft_strchr(l.str, CHAR))
